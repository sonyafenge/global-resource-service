...
readonly LOCAL_OUTPUT_ROOT="${GRS_ROOT}/${OUT_DIR:-_output}"
...
function grs::build::short_hash() {
  [[ $# -eq 1 ]] || {
    grs::log::error "Internal error.  No data based to short_hash."
    exit 2
  }
...
function grs::build::clean() {
  if [[ -d "${LOCAL_OUTPUT_ROOT}" ]]; then
    grs::log::status "Removing _output directory"
